...
% encuentra el punto de contacto automaticamente,
% obtiene el modulo de Young a partir de la curva de bajada (extend)
% y el trabajo de adhesion a partir de la curva de subida (retract).
...
start = 1; % Indico a partir de que indice buscar el punto de contacto
check = 1; % Elijo si quiero visualizar el ajuste que hizo (0: No / 1: Si)
...
E = zeros(length(archivos),1); % Defino vector generico para guardar resultados de Modulo de Young
re = zeros(length(archivos),1); % Defino vector generico para guardar coeficientes de correlacion de ajuste para elasticidad
if adhesion == 1
  A = zeros(length(archivos),1); % Defino vector generico para guardar resultados de Trabajo de Adhesion
  ra = zeros(length(archivos),1); % Defino vector generico para guardar coeficientes de correlacion de ajuste para adhesion
end
...
for i = 1:length(archivos)
  ...
  indCero = start; % Inicializo el indice del punto de contacto para entrar al bucle
  ...
  disp(['Curva ' num2str(i) ' de ' num2str(length(archivos)) ':']) % Muestra que curva esta analizando
  ...
  disp(['Quedan ' num2str(length(archivos)-i) ' curvas...']) % Muestra cuantas curvas faltan por analizar
  disp(' ');
...
save ([directorio 'E' extractAfter(getfield(archivos(1),'name'),10)],'E','-ascii'); % Guardo el vector de resultados de E en formato ascii
if adhesion == 1
  A = A(ra>=0.95&A>0); % Vector solo con los A positivos con coeficiente de correlacion mayor a 0.95
  save ([directorio 'A' extractAfter(getfield(archivos(1),'name'),10)],'A','-ascii'); % Guardo el vector de resultados de A en formato ascii
end

% Muestro resultados en consola:
disp('--- Resultados para esta celula ---');
disp(' ');
disp(['Modulo de Young: ' num2str(mean(E),'%.2f') ' kPa']);
if adhesion == 1
  disp(['Trabajo de Adhesion: ' num2str(mean(A),'%.2e') ' J/m2']);
end
disp(' ');
